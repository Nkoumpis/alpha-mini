...
class Network(nn.Module):
    ...
    def get_outputs(self, planes):
        # Build a batch‐of‐1 tensor, send to the same device as the model
        x = torch.tensor(planes, dtype=torch.float32).unsqueeze(0).to(
            torch.device("cuda" if self.use_gpu else "cpu")
        )
        # Forward pass
        p, v = self.forward(x)
        # Softmax over moves, extract as a numpy array
        probs = torch.softmax(p, dim=1)[0].detach().cpu().numpy()
        # v is [1,1], so v.item() is fine
        return probs, v.item()
